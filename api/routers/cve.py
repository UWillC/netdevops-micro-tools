...
def analyze_cve(req: CVEAnalyzeRequest):
    ...
    base_engine = CVEEngine(config=CVEEngineConfig(engine_version="0.3.4"))
    ...
    if _env_true("CVE_NVD_ENRICH") and matched_base:
        ...
        enriched_engine = CVEEngine(
            config=CVEEngineConfig(engine_version="0.3.4", enable_nvd_enrichment=True),
            providers=[
...
def check_cve(cve_id: str):
    ...
    engine = CVEEngine(config=CVEEngineConfig(engine_version="0.3.4"))
    engine.load_all()
